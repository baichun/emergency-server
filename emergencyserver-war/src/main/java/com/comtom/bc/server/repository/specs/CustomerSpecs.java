package com.comtom.bc.server.repository.specs;
				...
				for (Attribute<T, ?> attr : entity.getDeclaredAttributes()) {// 5
					...
					if (attrValue != null) {
						if (attr.getJavaType() == String.class) { // 7
							...
							predicates.add(cb.equal(root.get(attribute(entity, attr.getName(),
									attrValue.getClass())), attrValue)); // 10
						}
					}
